-- Update the reset_today_answers function to enforce a 90-day cooldown
...
BEGIN
    ...
    -- Enforce the 90-day limit (90 days)
    IF v_last_reset IS NOT NULL AND v_last_reset > NOW() - INTERVAL '90 days' THEN
        v_remaining_interval := (v_last_reset + INTERVAL '90 days') - NOW();
        
        -- Build a user-friendly time string (e.g. "85 Tagen", "12 Stunden", "3 Minuten")
        ...
        RAISE EXCEPTION 'Du kannst deine Antworten nur einmal alle 90 Tage zurücksetzen. Nächster Neustart möglich in %.', v_time_string;
    END IF;
