-- 1. Add perfect_tot_days_count column to profiles table
ALTER TABLE public.profiles 
  ADD COLUMN IF NOT EXISTS perfect_tot_days_count INTEGER DEFAULT 0 NOT NULL;
...
-- 4. Redefine check_and_unlock_milestones with perfect_tot_days_count
...
BEGIN
    ...
    FOR r_milestone IN SELECT id, trigger_type, trigger_value FROM public.milestones LOOP
        IF (
            ...
            (r_milestone.trigger_type = 'perfect_tot_days' AND v_perfect_tot_days >= r_milestone.trigger_value)
        ) THEN
...
CREATE TRIGGER on_profile_check_milestones
  AFTER UPDATE OF 
    ...
    perfect_match_days_count, perfect_tot_days_count
  ON public.profiles
    ...
    FOR a IN SELECT * FROM public.answers LOOP
        ...
        IF v_partner_id IS NOT NULL AND a.user_id < v_partner_id THEN
            ...
            IF FOUND THEN
                ...
                IF v_tot_new = v_tot_partner AND v_tot_new <> '' THEN
                    UPDATE public.profiles
                    SET perfect_tot_days_count = perfect_tot_days_count + 1
...
-- 7. Seed missing milestones
INSERT INTO public.milestones (name, description, icon, trigger_type, trigger_value) VALUES
  ('Jahreszeiten-Liebe', 'Haltet eine Antwort-Serie (Streak) von 90 Tagen am Stück.', '🍂', 'streak', 90),
  ('Gedanken-Zwillinge', 'Erzielt an 5 Tagen eine Übereinstimmung von 100% bei den Dies-oder-Das-Fragen.', '👯', 'perfect_tot_days', 5)
  ON CONFLICT DO NOTHING;
