...
-- 2. Create the updated function with excluded profile metric columns
           ...
           if (TG_TABLE_NAME = 'profiles') then
               if (TG_OP = 'DELETE') then
                   ...
                   for v_rec in select key, value from json_each_text(row_to_json(NEW)) loop
                       ...
                       if (v_rec.value IS DISTINCT FROM v_alter_wert) and v_rec.key NOT IN (
                           'updated_at', 
                           'total_answers', 
                           'total_matches', 
                           'nudge_count', 
                           'nudges_sent', 
                           'morning_answers_count', 
                           'night_answers_count', 
                           'lunch_answers_count', 
                           'last_minute_answers_count', 
                           'long_answers_count', 
                           'both_long_answers_count', 
                           'perfect_rankings_count', 
                           'perfect_match_days_count', 
                           'perfect_tot_days_count', 
                           'shares_count', 
                           'stats_views', 
                           'avatar_change_count', 
                           'streaks_rebuilt', 
                           'time_sync_5min_count', 
                           'time_sync_1min_count', 
                           'last_answer_reset_at'
                       ) then
