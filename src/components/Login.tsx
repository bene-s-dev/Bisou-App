...
export default function Login({ onLogin, initialMode = 'login' }: LoginProps) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      setMessage({ type: 'success', text: 'Registrierung erfolgreich! Bitte prüfe jetzt dein Postfach, um dein Konto zu bestätigen. ✨' });
      setLoading(false);
    }
  };
  ...
  return (
    <div className="w-full">
      {message && (
        <div className={`p-4 rounded-2xl mb-6 text-sm font-bold text-center animate-entrance bg-purple-50 text-[var(--secondary)] border border-purple-100`}>
          {message.text}
        </div>
      )}
      ...
      <button type="button" onClick={() => navigate('/')} className="w-full text-[10px] font-black text-[var(--muted)] uppercase tracking-[0.2em] mt-8 hover:text-[var(--text-main)] transition-colors text-center">← Zurück</button>
    </div>
  );
}
