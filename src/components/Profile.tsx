...
import { Camera, Pencil, Check, Bell, BellOff, Info, X, User as UserIcon, ChevronRight, ArrowLeft, Trash2, Share2, Copy, Download, Smartphone, Users, AlertTriangle, Sparkles, Monitor, Laptop, Tablet, Settings, Flame, ExternalLink, ShieldCheck, Shield } from 'lucide-react';
...
export default function Profile({ 
  ...
  const renderContent = () => {
    switch (activeTab) {
          ...
          <div className="flex flex-col items-center gap-4 animate-in fade-in zoom-in-95 duration-300 px-4">
            ...
            {profile?.partner_id ? (
              <div className="w-full flex flex-col gap-4">
                <div className="bg-white border-2 border-purple-50 rounded-[2.5rem] p-6 flex flex-col gap-6 shadow-sm">
                  <div className="flex items-center gap-4">
                    <div className="w-20 h-20 rounded-[2rem] bg-purple-50 flex items-center justify-center border-2 border-white shadow-md overflow-hidden relative">
                      {partnerProfile?.avatar_url ? (
                        <img src={partnerProfile.avatar_url} alt="P" className="w-full h-full object-cover" />
                      ) : (
                        <UserIcon className="w-8 h-8 text-[var(--secondary)]" />
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <h3 className="text-lg font-black text-[#1F1939] truncate">{partnerProfile?.display_name || 'Dein Partner'}</h3>
                      <div className="flex items-center gap-1.5 mt-0.5">
                        <Flame className="w-4 h-4 text-orange-500 fill-orange-500" />
                        <span className="text-xs font-bold text-orange-600 tracking-tight">{partnerDetails.streak} Tage Serie</span>
                      </div>
                    </div>
                  </div>
                  
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-purple-50/50 p-4 rounded-3xl border border-purple-100 flex flex-col items-center text-center gap-1">
                      <span className="text-[8px] font-black text-[var(--muted)] uppercase tracking-widest">Gemeinsam seit</span>
                      <span className="text-sm font-black text-[var(--secondary)]">{getDaysConnected()} Tagen</span>
                    </div>
                    <div className="bg-purple-50/50 p-4 rounded-3xl border border-purple-100 flex flex-col items-center text-center gap-1">
                      <span className="text-[8px] font-black text-[var(--muted)] uppercase tracking-widest">Registriert seit</span>
                ...
                <div className="p-2">
                  <button 
                    ...
                    className="w-full py-4 text-red-400 hover:text-red-500 font-black text-[10px] uppercase tracking-[0.2em] transition-colors"
                  >
              ...
              <div className="w-full flex flex-col gap-6">
                <div className="bg-white border-2 border-purple-50 rounded-[2.5rem] p-8 flex flex-col items-center text-center gap-6 shadow-sm">
                  <div className="w-16 h-16 bg-purple-50 rounded-2xl flex items-center justify-center text-[var(--secondary)] border-2 border-white shadow-sm">
                    <Share2 className="w-8 h-8" />
                  </div>
                  <div className="space-y-2">
                    <h3 className="text-lg font-black text-[#1F1939]">Teile deinen Code</h3>
                    <p className="text-xs font-bold text-[#4A4468] leading-relaxed opacity-70">
                      Damit dein Partner dich findet, teile diesen persönlichen Code mit ihm.
                    </p>
                  </div>
                  
                  <div className="flex items-center gap-2 w-full">
                    <div className="flex-1 bg-purple-50/50 border-2 border-purple-100 rounded-2xl p-4 flex items-center justify-center">
                      <span className="text-xl font-black text-[var(--secondary)] tracking-[0.2em] pt-0.5">{profile?.partner_code || '---'}</span>
                      <button onClick={() => { navigator.clipboard.writeText(profile?.partner_code || ''); showAlert("Code kopiert!", "success"); }} className="ml-2 p-2 bg-purple-50 text-[var(--secondary)] rounded-2xl hover:bg-purple-100 transition-all active:scale-90 flex items-center justify-center shrink-0">
                ...
                <div className="bg-white border-2 border-purple-50 rounded-[2.5rem] p-8 flex flex-col items-center text-center gap-6 shadow-sm">
                  ...
                  <div className="space-y-2">
                    <h3 className="text-lg font-black text-[#1F1939]">Code eingeben</h3>
                    <p className="text-xs font-bold text-[#4A4468] leading-relaxed opacity-70">
                      Oder gib hier den Partner-Code ein, den du erhalten hast.
                    </p>
                  </div>

                  <div className={`flex items-center gap-2 w-full ${shouldShake ? 'animate-vibrate' : ''}`}>
                    <input 
                      type="text" 
                      value={partnerCodeInput}
                      onChange={(e) => setPartnerCodeInput(e.target.value.toUpperCase())}
                      placeholder="CB-XXXXXX"
                      className="flex-1 bg-white border-2 border-purple-100 rounded-2xl p-4 text-center text-lg font-black text-[var(--secondary)] tracking-[0.2em] focus:border-[var(--secondary)] outline-none transition-colors"
                    />
                    <button 
                    ...
                    >
                      {isLinking ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <Check className="w-6 h-6" />}
                    </button>
                  </div>
                </div>
              </div>
      ...
      case 'notifications':
        return (
          <div className="flex flex-col items-center gap-4 animate-in fade-in zoom-in-95 duration-300 px-4">
             <h2 className="text-[10px] font-black text-[var(--secondary)] uppercase tracking-[0.2em] w-full text-center">Mitteilungen</h2>
             <div className="bg-white border-2 border-purple-50 rounded-[2.5rem] p-8 flex flex-col items-center text-center gap-6 shadow-sm w-full">
                <div className="w-16 h-16 bg-purple-50 rounded-2xl flex items-center justify-center text-[var(--secondary)] border-2 border-white shadow-sm">
                  {pushEnabled ? <Bell className="w-8 h-8" /> : <BellOff className="w-8 h-8" />}
                </div>
                <div className="space-y-2">
                  <h3 className="text-lg font-black text-[#1F1939]">Push-Benachrichtigungen</h3>
                  <p className="text-xs font-bold text-[#4A4468] leading-relaxed opacity-70">
                    Lass dich benachrichtigen, wenn dein Partner geantwortet hat oder neue Fragen verfügbar sind.
                  </p>
                </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-xl ${pushEnabled ? 'bg-green-50 text-green-500' : 'bg-purple-50 text-[var(--secondary)]'}`}>
                      {pushEnabled ? <Check className="w-4 h-4" /> : <Settings className="w-4 h-4" />}
                    </div>
                    <span className="text-xs font-black text-[#1F1939] uppercase tracking-wide">{pushEnabled ? 'Aktiviert' : 'Deaktiviert'}</span>
                  </div>
                  <div className={`w-10 h-6 rounded-full transition-colors relative ${pushEnabled ? 'bg-green-500' : 'bg-gray-200'}`}>
                    <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${pushEnabled ? 'left-5' : 'left-1'}`} />
                  </div>
                </button>
             </div>
          </div>
      ...
      case 'install':
        ...
        const isDesktop = !isIOS && !/android/.test(navigator.userAgent.toLowerCase());
        return (
          <div className="flex flex-col items-center gap-4 animate-in fade-in zoom-in-95 duration-300 px-4">
            <h2 className="text-[10px] font-black text-[var(--secondary)] uppercase tracking-[0.2em] w-full text-center">App Installation</h2>
            <div className="bg-white border-2 border-purple-50 rounded-[2.5rem] p-8 flex flex-col items-center text-center gap-6 shadow-sm w-full">
                <div className="w-16 h-16 bg-purple-50 rounded-2xl flex items-center justify-center text-[var(--secondary)] border-2 border-white shadow-sm">
                  <Smartphone className="w-8 h-8" />
                </div>
                <div className="space-y-2">
                  <h3 className="text-lg font-black text-[#1F1939]">Bisou auf dem Homescreen</h3>
                  <p className="text-xs font-bold text-[#4A4468] leading-relaxed opacity-70">
                    Nutze Bisou wie eine echte App. Ohne Browser-Leiste, direkt von deinem Startbildschirm.
                  </p>
                </div>
        ...
        return (
          <div className="flex flex-col items-center gap-4 animate-in fade-in zoom-in-95 duration-300 px-2 w-full">
            <div className="flex items-center justify-center gap-2 relative w-full max-w-xl">
              <h2 className="text-[10px] font-black text-[var(--secondary)] uppercase tracking-[0.2em]">INFORMATIONEN</h2>
              {isDevMode && (
                <button
            ...
            <div className="bg-white/80 backdrop-blur-md rounded-[32px] px-4 py-6 border-2 border-blue-100 w-full max-w-xl overflow-hidden animate-all duration-300 mx-auto">
              <div className="grid grid-cols-[auto_1fr_12px] gap-x-4 gap-y-3 items-center">
                ...
                <div className="contents">
                  ...
                  <div className="border-b border-purple-50/50 pb-2 h-full w-full" />
                </div>
              ...
              <button 
                ...
                className="w-full mt-6 py-4 bg-emerald-50 text-emerald-600 font-black text-[10px] uppercase tracking-[0.2em] rounded-2xl active:scale-95 transition-all shadow-sm border border-emerald-100 flex items-center justify-center gap-2"
              >
            ...
            <div className="flex flex-col gap-2 w-full max-w-xl mt-2">
              <button
                onClick={() => setShowServices(true)}
                className="w-full py-4 bg-blue-50 text-blue-600 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-100 transition-colors flex items-center justify-center gap-1.5 active:scale-95 border border-blue-100"
              >
                Verwendete Dienste
              </button>
            </div>
        ...
        return createPortal(
          <div className="fixed inset-0 z-[200] flex items-center justify-center sm:p-4">
            <div 
              ...
              onClick={() => navigate('/profile?tab=main')}
            />
            <div className="relative w-full h-full sm:max-w-lg sm:h-[85vh] sm:max-h-[850px] bg-white border border-purple-100 sm:rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-10 sm:slide-in-from-bottom-0 duration-300">
              <Intro isIntroOnly onComplete={() => navigate('/profile?tab=main')} />
            </div>
          </div>,
          document.body
        );
            ...
            {[
              ...
              <button 
                ...
                onClick={() => {
                  ...
                  else if (item.id === 'notifications' && !isDevMode) {
                    // Do nothing for normal users
                  }
                  else navigate(`/profile?tab=${item.id}`);
                }} 
                ...
                } ${item.id === 'notifications' && !isDevMode ? 'opacity-50 grayscale-[0.5]' : ''}`}
              >
                <div className="flex items-center gap-3">
                  <div className={`p-2.5 rounded-2xl ${item.isDanger ? 'bg-red-50 text-red-500' : 'bg-purple-50 text-[var(--secondary)]'}`}>
                    <item.icon className="w-5 h-5" />
                  </div>
                  <div className="flex flex-col items-start">
                    <span className={`text-xs font-black uppercase tracking-wide ${item.isDanger ? 'text-red-500' : 'text-[#1F1939]'}`}>{item.label}</span>
                    {item.id === 'notifications' && !isDevMode && (
                      <span className="bg-amber-100 text-amber-600 text-[7px] font-black px-1.5 py-0.5 rounded-full uppercase tracking-widest mt-0.5 border border-amber-200">
                        Bald verfügbar
                      </span>
                    )}
                  </div>
                </div>
                <ChevronRight className={`w-4 h-4 ${item.isDanger ? 'text-red-200' : (item.id === 'notifications' && !isDevMode ? 'text-purple-100' : 'text-purple-200')}`} />
              </button>
            ))}
          </div>
        );
  ...
  return (
    <div className="flex flex-col h-full bg-[#F8F7FF] relative select-none">
      <div className="bg-aura" />
      <header className="pt-6 pb-4 px-4 flex flex-col items-center gap-6 shrink-0 relative z-10">
        <div className="flex items-center justify-center w-full relative h-[40px]">
           {activeTab !== 'main' && (
            <button 
              onClick={() => navigate('/profile?tab=main')}
              className="absolute left-0 p-2 rounded-full bg-white border border-purple-100 shadow-sm active:scale-95 transition-all"
            >
              <ArrowLeft className="w-4 h-4 text-[var(--secondary)]" />
            </button>
          )}
          
          <h1 className="text-xl font-semibold text-[var(--text-main)] tracking-tight" style={{ fontFamily: 'Fraunces, serif' }}>
            Profil
          </h1>
        </div>
          ...
          {isEditingName ? (
            <div className="flex items-center gap-2 bg-white border-2 border-[var(--secondary)] rounded-full px-4 py-1.5 shadow-md animate-in zoom-in-95 duration-200">
              <input
                ...
                className="bg-transparent text-[15px] font-black text-[var(--secondary)] uppercase tracking-[0.1em] outline-none w-32 pt-0.5"
              />
              <button onClick={handleNameSave} className="p-1 text-green-500 hover:bg-green-50 rounded-full transition-colors">
      ...
      <DeleteAccountModal 
        ...
        onConfirm={async () => {
          // Actual deletion logic is handled in Profile.tsx if needed, 
          // or we just call the prop if passed. 
