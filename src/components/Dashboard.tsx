...
export default function Dashboard({ 
  ...
  }, [currentNewMilestone, toastPaused]);
    ...
    const checkNewMilestones = async () => {
      try {
        ...
        if (newUnlocks.length > 0) {
          ...
          // to prevent retroactively showing toast messages for historical calculations/migrations.
          ...
          if (recentUnlocks.length > 0) {
            setNewMilestones(prev => {
              ...
              const filtered = recentUnlocks.filter(um => !existingIds.includes(um.id));
              return [...prev, ...filtered];
            });
          }

          // Update cache with all unlocked IDs (both recent and old) so they aren't processed again
          ...
          <div className="absolute inset-0 pointer-events-none rounded-[2rem] overflow-visible">
            ...
            >
              ...
              <rect
                ...
                rx="32"
                fill="none"
                stroke="url(#toast-border-gradient)"
                strokeWidth="1.8"
                pathLength="100"
                strokeDasharray="100"
